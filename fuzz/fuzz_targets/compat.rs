...
fuzz_target!(|data: &[u8]| {
    ...
    match (&cracode_v1, &cracode_v2) {
        (Err(e), _) if e.to_string() == "the size limit has been reached" => {}
        (_, Err(cracode::error::DecodeError::LimitExceeded)) => {}
        (Ok(cracode_v1), Ok((cracode_v2, _))) if cracode_v1 != cracode_v2 => {
            println!("Bytes:      {:?}", data);
            println!("Cracode V1: {:?}", cracode_v1);
            println!("Cracode V2: {:?}", cracode_v2);
