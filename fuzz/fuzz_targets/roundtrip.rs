...
#[derive(cracode::Decode, cracode::Encode, PartialEq, Debug)]
enum AllTypes {
...
fuzz_target!(|data: &[u8]| {
    let config = cracode::config::standard().with_limit::<1024>();
    let result: Result<(AllTypes, _), _> = cracode::decode_from_slice(data, config);

    if let Ok((before, _)) = result {
        let encoded = cracode::encode_to_vec(&before, config).expect("round trip");
        let (after, _) = cracode::decode_from_slice(&encoded, config).unwrap();
