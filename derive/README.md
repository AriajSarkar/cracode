...
The derive crate for cracode. Implements `cracode::Encode` and `cracode::Decode`.
...
For testing purposes, all generated code is outputted to the current `target/generated/cracode` folder, under file name `<struct/enum name>_Encode.rs` and `<struct/enum name>_Decode.rs`. This can help with debugging.
