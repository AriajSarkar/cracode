...
fn test_refcell_already_borrowed() {
    ...
    let result = cracode::encode_into_slice(&cell, &mut slice, cracode::config::standard())
        .expect_err("Encoding a borrowed refcell should fail");

    match result {
        cracode::error::EncodeError::RefCellAlreadyBorrowed {
            ..
        } => {} // ok
...
fn test_slice() {
    ...
    cracode::encode_into_slice(input, &mut buffer, cracode::config::standard()).unwrap();
    assert_eq!(&buffer[..8], &[7, 1, 2, 3, 4, 5, 6, 7]);

    let (output, len): (&[u8], usize) =
        cracode::borrow_decode_from_slice(&buffer[..8], cracode::config::standard()).unwrap();
...
fn test_option_slice() {
    ...
    let n = cracode::encode_into_slice(input, &mut buffer, cracode::config::standard()).unwrap();
    assert_eq!(&buffer[..n], &[1, 7, 1, 2, 3, 4, 5, 6, 7]);

    let (output, len): (Option<&[u8]>, usize) =
        cracode::borrow_decode_from_slice(&buffer[..n], cracode::config::standard()).unwrap();
    assert_eq!(input, output);
    assert_eq!(len, n);
...
fn test_str() {
    ...
    let (output, len): (&str, usize) =
        cracode::borrow_decode_from_slice(&buffer[..12], cracode::config::standard()).unwrap();
    assert_eq!(input, output);
    assert_eq!(len, 12);
}
...
fn test_array() {
    ...
    let (output, len): ([u8; 10], usize) =
        cracode::decode_from_slice(&buffer[..11], cracode::config::standard()).unwrap();
    assert_eq!(input, output);
    assert_eq!(len, 10);

    let mut buffer = [0u8; 32];
    let input: [u8; 1] = [1];
    let config = cracode::config::standard().with_fixed_int_encoding().with_little_endian();
    let len = cracode::encode_into_slice(input, &mut buffer, config).unwrap();
    assert_eq!(len, 1);
    assert_eq!(&buffer[..1], &[1]);

    let (output, len): ([u8; 1], usize) = cracode::decode_from_slice(&buffer[..9], config).unwrap();
    assert_eq!(len, 1);
    assert_eq!(input, output);
}
...
fn test_duration_out_of_range() {
    ...
    cracode::encode_into_slice((u64::MAX, u32::MAX), &mut input, cracode::config::standard())
        .unwrap();

    let result: Result<(std::time::Duration, usize), _> =
        cracode::decode_from_slice(&input, cracode::config::standard());
...
fn test_duration_wrapping() {
    ...
    cracode::encode_into_slice((u64::MAX - 4, u32::MAX), &mut input, cracode::config::standard())
        .unwrap();

    let (result, _): (std::time::Duration, _) =
        cracode::decode_from_slice(&input, cracode::config::standard()).unwrap();
