...
impl MemCache {
        ...
        T: Send + Sync + serde::Serialize,
    {
        ...
        let encoded = cracode::serde::encode_to_vec(cache_data, config)?;
    ...
    fn get_data<T>(&self, key: &Uuid) -> Result<T, cracode::error::DecodeError>
    where
        T: Send + Sync + DeserializeOwned,
    {
        ...
        let (decoded, _len): (T, usize) =
            cracode::serde::decode_from_slice(&cache_item.payload[..], config)?;
        Ok(decoded)
    }
}
