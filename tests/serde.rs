...
#[derive(Serialize, Deserialize, cracode::Encode, cracode::Decode)]
...
#[derive(Serialize, Deserialize, cracode::Encode, cracode::Decode, PartialEq, Debug)]
...
fn test_serde_round_trip() {
    ...
    // validate cracode working
    let bytes = cracode::serde::encode_to_vec(
        SerdeRoundtrip {
            a: 15,
            b: 15,
            c: TupleS(2.0, 3.0, 4.0),
    ...
    let (result, len): (SerdeRoundtrip, usize) =
        cracode::serde::decode_from_slice(&bytes, cracode::config::standard()).unwrap();
...
fn test_serialize_deserialize_borrowed_data() {
    ...
    let len = cracode::serde::encode_into_slice(&input, &mut result, cracode::config::standard())
        .unwrap();
    let result = &result[..len];
    assert_eq!(result, expected);

    let result = cracode::serde::encode_to_vec(&input, cracode::config::standard()).unwrap();

    assert_eq!(result, expected);

    let (output, len): (SerdeWithBorrowedData, usize) =
        cracode::serde::borrow_decode_from_slice(&result, cracode::config::standard()).unwrap();
    assert_eq!(
...
fn test_serialize_deserialize_owned_data() {
    ...
    let (output, len): (SerdeWithOwnedData, usize) =
        cracode::serde::decode_from_slice(&result, cracode::config::standard()).unwrap();
    assert_eq!(
        SerdeWithOwnedData {
            b: 0, // remember: b is skipped
            ..input
...
mod derive {
    ...
    fn test_serde_derive() {
        ...
        where
            T: cracode::Encode + cracode::Decode<()> + PartialEq + core::fmt::Debug,
        {
            let mut slice = [0u8; 100];
            let len = cracode::encode_into_slice(&start, &mut slice, cracode::config::standard())
                .unwrap();
