...
fn test_std_cursor() {
    ...
    let foo: Foo = cracode::decode_from_std_read(&mut cursor, cracode::config::standard()).unwrap();
...
fn test_std_file() {
    ...
    let bytes_written = cracode::encode_into_std_write(
        Foo {
            a: 30,
            b: 50,
        },
    ...
    let foo: Foo = cracode::decode_from_std_read(&mut file, cracode::config::standard()).unwrap();
...
fn test_std_commons() {
    ...
    let len = cracode::encode_into_slice(cstr, &mut buffer, config).unwrap();
    let (decoded, len): (CString, usize) =
        cracode::decode_from_slice(&buffer[..len], config).unwrap();
    assert_eq!(cstr, decoded.as_c_str());
    assert_eq!(len, 12);

    // Path
    let path = Path::new("C:/Program Files/Foo");
    let len = cracode::encode_into_slice(path, &mut buffer, config).unwrap();
    let (decoded, len): (&Path, usize) =
        cracode::borrow_decode_from_slice(&buffer[..len], config).unwrap();
...
fn test_decode_borrow_str_in_array() {
    let (strs, _): (Vec<&str>, usize) = cracode::borrow_decode_from_slice(
    ...
    let (strs, _): ([&str; 3], usize) = cracode::borrow_decode_from_slice(
