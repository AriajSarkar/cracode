...
- [`with_big_endian`](https://docs.rs/cracode/2/cracode/config/struct.Configuration.html#method.with_big_endian)
- [`with_little_endian`](https://docs.rs/cracode/2/cracode/config/struct.Configuration.html#method.with_little_endian)
...
- During deserialization, values other than 0 and 1 will result in an error [`DecodeError::InvalidBooleanValue`](https://docs.rs/cracode/2/cracode/error/enum.DecodeError.html#variant.InvalidBooleanValue)
...
- Invalid Unicode characters can be acquired via unsafe code, this is handled as:
  ...
  - during deserialization: an error is raised [`DecodeError::InvalidCharEncoding`](https://docs.rs/cracode/2/cracode/error/enum.DecodeError.html#variant.InvalidCharEncoding)
...
let encoded = cracode::encode_to_vec(tuple, cracode::config::legacy()).unwrap();
...
Cracode currently supports 2 different types of `IntEncoding`. With the default config, `VarintEncoding` is selected.
...
See the documentation of [VarintEncoding](https://docs.rs/cracode/2/cracode/config/struct.Configuration.html#method.with_variable_int_encoding) for more information.
...
See the documentation of [FixintEncoding](https://docs.rs/cracode/2/cracode/config/struct.Configuration.html#method.with_fixed_int_encoding) for more information.
...
// SomeEnum::A
let encoded = cracode::encode_to_vec(SomeEnum::A, cracode::config::legacy()).unwrap();
assert_eq!(encoded.as_slice(), &[
...
// SomeEnum::B(0)
let encoded = cracode::encode_to_vec(SomeEnum::B(0), cracode::config::legacy()).unwrap();
assert_eq!(encoded.as_slice(), &[
...
let encoded = cracode::encode_to_vec(SomeEnum::C { value: 0u32 }, cracode::config::legacy()).unwrap();
assert_eq!(encoded.as_slice(), &[
...
let encoded = cracode::encode_to_vec(data, cracode::config::legacy()).unwrap();
assert_eq!(encoded.as_slice(), &[
...
- Duplicate keys are not checked in cracode, but may be resulting in an error when decoding a container from a list of pairs.
...
let encoded = cracode::encode_to_vec(list, cracode::config::legacy()).unwrap();
assert_eq!(encoded.as_slice(), &[
...
- Cracode will serialize the entries based on the iterator order.
...
- During serialization, the string is encoded as a sequence of the given bytes.
  - Rust strings are UTF-8 encoded by default, but this is not enforced by cracode
- No normalization or transformation of text
- If an invalid UTF-8 sequence is encountered during decoding, an [`DecodeError::Utf8`](https://docs.rs/cracode/2/cracode/error/enum.DecodeError.html#variant.Utf8) error is raised

```rust
let str = "Hello 🌍"; // Mixed ASCII and Unicode

let encoded = cracode::encode_to_vec(str, cracode::config::legacy()).unwrap();
assert_eq!(encoded.as_slice(), &[
...
let encoded = cracode::encode_to_vec(arr, cracode::config::legacy()).unwrap();
assert_eq!(encoded.as_slice(), &[
