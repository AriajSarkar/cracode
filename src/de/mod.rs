...
///
/// This trait will be automatically implemented with unbounded `Context` if you enable the `derive` feature and add `#[derive(cracode::Decode)]` to your type. Note that if the type contains any lifetimes, `BorrowDecode` will be implemented instead.
///
/// # Implementing this trait manually
///
/// If you want to implement this trait for your type, the easiest way is to add a `#[derive(cracode::Decode)]`, build and check your `target/generated/cracode/` folder. This should generate a `<Struct name>_Decode.rs` file.
///
/// For this struct:
///
/// ```
...
/// #     pub y: f32,
/// # }
/// impl<Context> cracode::Decode<Context> for Entity {
///     fn decode<D: cracode::de::Decoder<Context = Context>>(
///         decoder: &mut D,
///     ) -> core::result::Result<Self, cracode::error::DecodeError> {
///         Ok(Self {
///             x: cracode::Decode::decode(decoder)?,
///             y: cracode::Decode::decode(decoder)?,
///         })
///     }
/// }
/// impl<'de, Context> cracode::BorrowDecode<'de, Context> for Entity {
///     fn borrow_decode<D: cracode::de::BorrowDecoder<'de, Context = Context>>(
///         decoder: &mut D,
///     ) -> core::result::Result<Self, cracode::error::DecodeError> {
///         Ok(Self {
///             x: cracode::BorrowDecode::borrow_decode(decoder)?,
///             y: cracode::BorrowDecode::borrow_decode(decoder)?,
///         })
///     }
/// }
/// ```
...
/// #         decoder: &mut D,
/// #     ) -> core::result::Result<Self, cracode::error::DecodeError> {
/// let x: u8 = cracode::Decode::<Context>::decode(decoder)?;
/// let x = <u8 as cracode::Decode::<Context>>::decode(decoder)?;
/// #         Ok(Foo)
/// #     }
/// # }
...
///
/// This trait will be automatically implemented if you enable the `derive` feature and add `#[derive(cracode::Decode)]` to a type with a lifetime.
...
pub trait Decoder: Sealed {
    ...
    /// This is to ensure that cracode won't allocate several GB of memory while constructing the container.
    ///
    ...
    ///     }
    /// }
    /// impl<'de, Context, T: cracode::BorrowDecode<'de, Context>> cracode::BorrowDecode<'de, Context> for Container<T> {
    ///     fn borrow_decode<D: cracode::de::BorrowDecoder<'de, Context = Context>>(
    ///         decoder: &mut D,
    ///     ) -> core::result::Result<Self, cracode::error::DecodeError> {
