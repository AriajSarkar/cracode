...
//! - [encode_to_vec]
//!
//! For interop with cracode's [Decode]/[Encode], you can use:
//! - [Compat]
//! - [BorrowCompat]
//!
//! For interop with cracode's `derive` feature, you can use the `#[cracode(with_serde)]` attribute on each field that implements serde's traits.
//!
//! ```
//! # #[cfg(feature = "derive")]
//! # mod foo {
...
//!     Unit(#[cracode(with_serde)] SerdeType),
//!     Struct {
//!         #[cracode(with_serde)]
//!         serde: SerdeType,
//!     },
...
//! # Known issues
//!
//! Because cracode is a format without meta data, there are several known issues with serde's attributes. Please do not use any of the following attributes if you plan on using cracode, or use cracode's own `derive` macros.
//! - `#[serde(flatten)]`
//! - `#[serde(skip)]`
...
//! - `#[serde(untagged)]`
//!
//! **Using any of the above attributes can and will cause issues with cracode and will result in lost data**. Consider using cracode's own derive macro instead.
//!
...
//! - Serde has chosen to not have a MSRV ([source](https://github.com/serde-rs/serde/pull/2257)). We think MSRV is important, cracode 1 still compiles with rust 1.18.
...
//! - We believe that less dependencies is better, and that you should be able to choose your own dependencies. If you disable all features, cracode 2 only has 1 dependency. ([`unty`], a micro crate we manage ourselves)
//!
...
pub enum DecodeError {
    /// Cracode does not support serde's `any` decoding feature.
    ///
    /// See the "known issues" list in the serde module for more information on this.
    AnyNotSupported,

    /// Cracode does not support serde identifiers
    IdentifierNotSupported,

    /// Cracode does not support serde's `ignored_any`.
    ///
    /// See the "known issues" list in the serde module for more information on this.
    ...
    /// Custom serde error but cracode is unable to allocate a string. Set a breakpoint where this is thrown for more information.
    #[cfg(not(feature = "alloc"))]
    CustomError,
}
...
pub enum EncodeError {
    /// Serde provided cracode with a sequence without a length, which is not supported in cracode
    SequenceMustHaveLength,

    /// [Serializer::collect_str] got called but cracode was unable to allocate memory.
    #[cfg(not(feature = "alloc"))]
    CannotCollectStr,
