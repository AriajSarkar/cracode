...
//! Cracode is a crate for encoding and decoding using a tiny binary
//! serialization strategy.  Using it, you can easily go from having
//! an object in memory, quickly serialize it to bytes, and then
//! deserialize it back just as fast!
//!
//! If you're coming from cracode 1, check out our [migration guide](migration_guide/index.html)
//!
//! # Serde
//!
//! Starting from cracode 2, serde is now an optional dependency. If you want to use serde, please enable the `serde` feature. See [Features](#features) for more information.
//!
//! # Features
//!
...
//! |serde | No     | Yes (MSRV reliant on serde)|`Compat` and `BorrowCompat`, which will work for all types that implement serde's traits|serde-specific encode/decode functions in the [serde] module|Note: There are several [known issues](serde/index.html#known-issues) when using serde and cracode|
//!
//! # Which functions to use
//!
//! Cracode has a couple of pairs of functions that are used in different situations.
//!
...
//!
//! **Note:** If you're using `serde`, use `cracode::serde::...` instead of `cracode::...`
//!
//! # Example
//!
//! ```rust
...
//! );
//!
//! let length = cracode::encode_into_slice(
//!     input,
//!     &mut slice,
...
//! let decoded: (u8, u32, i128, char, [u8; 4]) = cracode::decode_from_slice(slice, cracode::config::standard()).unwrap().0;
//!
