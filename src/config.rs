//! The config module is used to change the behavior of cracode's encoding and decoding logic.
//!
//! *Important* make sure you use the same config for encoding and decoding, or else cracode will not work properly.
//!
//! To use a config, first create a type of [Configuration]. This type will implement trait [Config] for use with cracode.
//!
//! ```
...
/// The Configuration struct is used to build cracode configurations. The [Config] trait is implemented
...
/// The default config for cracode 2.0. By default this will be:
/// - Little endian
...
/// Creates the "legacy" default config. This is the default config that was present in cracode 1.0
/// - Little endian
...
impl<E, I, L> Configuration<E, I, L> {
    /// Makes cracode encode all integer types in big endian.
    pub const fn with_big_endian(self) -> Configuration<BigEndian, I, L> {
        generate()
    }

    /// Makes cracode encode all integer types in little endian.
    pub const fn with_little_endian(self) -> Configuration<LittleEndian, I, L> {
        generate()
    }

    /// Makes cracode encode all integer types with a variable integer encoding.
    ///
...
/// Indicates a type is valid for controlling the cracode configuration
pub trait Config:
